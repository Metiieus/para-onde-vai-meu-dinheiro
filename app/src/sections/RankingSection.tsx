...
import { ArrowLeft, Filter, TrendingUp, Download, Calendar, Trophy } from 'lucide-react';
...
export function RankingSection({ onVoltar, onSelecionarPolitico }: RankingSectionProps) {
  const { ranking, loading, categoriaAtual, ano, setCategoriaAtual, setAno } = useRanking();
      ...
      <header className="bg-white border-b border-slate-100 sticky top-0 z-50">
        <div className="container mx-auto px-4 h-14 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <Button variant="ghost" size="icon" onClick={onVoltar} className="hover:bg-slate-50">
            ...
            <div>
              ...
              <p className="text-xs text-slate-400">{categoriaInfo?.nome} - {ano}</p>
            </div>
          </div>
          <Button variant="outline" size="sm" className="border-slate-200 text-slate-500">
          ...
          <CardContent className="p-4">
            <div className="flex flex-col md:flex-row gap-4">
              ...
              <div className="flex items-end">
                <Tabs value={visualizacao} onValueChange={(v) => setVisualizacao(v as 'lista' | 'grafico')}>
                  <TabsList className="bg-slate-100">
                    <TabsTrigger value="lista" className="data-[state=active]:bg-white data-[state=active]:text-emerald-600">Lista</TabsTrigger>
                    <TabsTrigger value="grafico" className="data-[state=active]:bg-white data-[state=active]:text-emerald-600">Gráfico</TabsTrigger>
                  </TabsList>
                </Tabs>
              </div>
            </div>
        ...
        <div className="grid lg:grid-cols-3 gap-6">
          <div className="lg:col-span-2">
            {visualizacao === 'lista' ? (
              ...
              <GraficoBarras dados={dadosGrafico} titulo={`Top 10 - ${categoriaInfo?.nome}`} altura={350} />
            )}
          </div>
              ...
              <CardContent className="space-y-3">
                {ranking.slice(0, 3).map((politico, index) => (
                  <div key={politico.id} className="flex items-center gap-3">
                    ...
                    <p className="text-sm font-semibold text-rose-500">{formatarMoeda(politico.valorTotal)}</p>
                  </div>
                ))}
              </CardContent>
            </Card>
            ...
            <Card className="bg-slate-50 border-slate-100">
              <CardContent className="p-4">
                <p className="text-xs text-slate-400">
                  <strong className="text-slate-500">Fonte:</strong> Câmara dos Deputados
                </p>
                <p className="text-xs text-slate-400 mt-1">
                  <strong className="text-slate-500">CEAP:</strong> Cota parlamentar
                </p>
              </CardContent>
            </Card>
          </div>
